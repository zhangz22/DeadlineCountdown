...
public class SideBarPanel extends JPanel {
    ...
    public static final class PANEL {
        ...
        public static final String THEME_PANEL = "theme";
    }
    ...
    public SideBarPanel(HashMap<String, Deadline> deadlines, String username, GUIController parent) {
        ...
        for (Deadline d: allDeadlines.values()) {
            this.allCourseNames.add(d.getCourseName());
        }
            ...
            if (e.getStateChange() == ItemEvent.SELECTED) {
                ...
                if (source instanceof JComboBox) {
                    ...
                    if (parent.getFrame().getText("date2").equals(selectedItem)) {
                        ...
                        sortOrder = Comparator.comparing(Deadline::getTime);
                    } else if (parent.getFrame().getText("name").equals(selectedItem)) {
                        sortOrder = Comparator.comparing(o -> (o.getCourseName() + " " + o.getName()));
                    }
                }
        ...
        if (parent.getSettings().isShowPastDeadlines()) pastDeadlinesBox.setSelected(true);
        this.pastDeadlinesBox.addItemListener(e -> {
            parent.setSettings(Settings.SUPPORTED.SHOW_PAST_DEADLINES, e.getStateChange() == ItemEvent.SELECTED);
            updateSummaryPanel();  // Refresh after changing past deadlines options
            parent.refresh();
        });
        ...
        // theme panel
        JPanel themePanel = new ThemePanel(parent);
        ...
        this.lowerPart.add(themePanel, PANEL.THEME_PANEL);
    ...
    private void addUpperPartComponents() {
        // Top blank area
        this.add(DeadlineCountdownFactory.createEmptyArea(10, true));

        // main Icon
        JPanel northWest = DeadlineCountdownFactory.createPanel(SIDEBAR_WIDTH, 55,true);
        ...
        this.add(DeadlineCountdownFactory.createEmptyArea(3, true));
        ...
        welcomeTextArea.setFont(new Font(ViewerFont.XHEI, Font.PLAIN, 20));
        ...
        newDeadlineButton.setFont(new Font(ViewerFont.XHEI, Font.PLAIN, 20));
        newDeadlineButton.addMouseListener(DeadlineCountdownFactory.getHoverEffect(newDeadlineButton, parent.getFrame().getTheme().SIDEBAR_HOVER()));
        ...
        this.add(DeadlineCountdownFactory.createEmptyArea(2, true));
    ...
    public void addDeadline(Deadline deadline) {
        ...
        this.allDeadlines.put(deadline.getCourseName().replace("Spring 2019     ", "") + ": " + deadline.getName(),
                deadline);
        this.allCourseNames.add(deadline.getCourseName());
        this.addNewDeadlinePanel.addCourse(deadline.getCourseName());
        Log.debug("DEBUG: [SideBarPanel_] {" + deadline.toString() + "} will be added to sidebar.", Log.ANSI_BLUE);
    ...
    public void addCourse(String courseName) {
        ...
        Log.debug("DEBUG: [SideBarPanel_] adding an empty course {"
                + courseName + "} to list.", Log.ANSI_GREEN);
    }
     ...
     * @param status       a string to represent the status
    ...
    void handleAddDeadlineSignal(String course, String deadlineName, int year,
                                 int month, int day, int hour, int minute, String status, String link) {
        this.parent.addDeadline(course, deadlineName, year, month,day, hour, minute, status, link);
    }
    ...
    public void updateSummaryPanel() {
        ...
        list.sort(sortOrder);
        for (Deadline d: list) {
            if (d.isBefore(CalendarWrapper.now()) && !parent.getSettings().isShowPastDeadlines()) {
                continue;
            }
            if (parent.isIgnoring(d.getCourseName())) {
                continue;
            }
            this.summaryPanel.add(new DeadlineInfoBlock(this,
                    d.getCourseName().replace("Spring 2019     ", "") + ": " + d.getName(), d));
            if (i != list.size() - 1) {
                this.summaryPanel.add(DeadlineCountdownFactory.createEmptyArea(13, true));
            }
            i++;
        }
